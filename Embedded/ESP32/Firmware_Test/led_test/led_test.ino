 ...
 *   - Two chase modes: (1) wrapping loop, (2) reset at the end of the strip.
 *   - The step delay is adjustable in ms (default 120).
...
void printStatus() {
  Serial.printf(">> LEDs:%d  Mode:%d (%s)  Delay:%lu ms  Frames:%d  %s\n",
                chaseLeds,
                chaseMode,
                chaseMode == 2 ? "reset at end" : "wrapping loop",
                chaseDelay,
                frameCount(),
...
void printMenu() {
  ...
  Serial.println(F("  d <ms>    -> set step delay in milliseconds (default 120)"));
...
void setMode(int m) {
  if (m != 1 && m != 2) {
    Serial.println(F("!! Chase mode must be 1 or 2"));
    return;
  }
  chaseMode = m;
  restartAnimation();
  Serial.printf(">> Chase mode %d (%s), %d frames\n",
                chaseMode, chaseMode == 2 ? "reset at end" : "wrapping loop", frameCount());
}
